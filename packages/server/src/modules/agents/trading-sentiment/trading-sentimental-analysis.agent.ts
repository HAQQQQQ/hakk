...
//      */
//     async execute<TradingSentimentAnalysis>(journalEntry: string): Promise<TradingSentimentAnalysis> {
...
// base-trading-sentiment-agent.ts
import { Injectable } from "@nestjs/common";
import { BaseAgent } from "../base.agent";
import { OpenAIClientService } from "../../openai/openai-client.service";
import { AgentName } from "../agent.factory";
...
export abstract class BaseTradingSentimentAgent<TParams, TResult> extends BaseAgent {
	constructor(
		...
		super(openaiClient, agentName, systemMessage, schemaName, toolDescription);
	}
	...
	/**
	 * Abstract method to build a prompt from the provided parameters
	 * Each subclass will implement this based on its specific parameters
	 */
	abstract buildPrompt(params: TParams): string;
